...
[Not needed for DDI tests at the moment:
...
2. If running against localhost, then no need to set `TEST_ENVIRONMENT_ROOT_URL` as it will default to `docker.host.internal:3002`.  Instead make sure the application container is running with `docker-compose up --build` in the root folder of this repository

3. From the directory containing the dockerfile run `docker-compose run --rm wdio-cucumber`. This will run the acceptance tests against the DDI web service. Currently it is your responsibility for a local env (portal + api as a minimum) to be running locally.
...
- `I select the radio option with the value "([^"]*)?" from the radio group "([^"]*)?"` <br>Select a radio group option based on its value (not display text) and radio group name
