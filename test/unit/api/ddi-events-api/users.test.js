...
describe('users test', () => {
  ...
  describe('createUser', () => {
    ...
    test('createUser should return the response object given api request failed with 500 error', async () => {
      ...
      await expect(createUser(newUser, adminUser)).rejects.toThrow(ApiErrorFailure)
      await expect(createUser(newUser, adminUser)).rejects.not.toThrow(ApiConflictError)
    })

    test('should fail given pseudonym is missing', async () => {
      const newUser = {
        username: 'Joel.Murphy78'
      }
      await expect(createUser(newUser, adminUser)).rejects.toThrow()
    })
