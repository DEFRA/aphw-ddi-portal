...
describe('cdo tasklist session helpers', () => {
  const mockRequest = {
    yar: {
      get: jest.fn(),
      set: jest.fn()
  ...
  test('getVerificationPayload', () => {
    mockRequest.yar.get.mockReturnValue(buildVerificationPayload({

    }))

    const enforcementDetails = getVerificationPayload(mockRequest, 'createdCdo', 'enforcementDetails')
