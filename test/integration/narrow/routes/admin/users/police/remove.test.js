...
describe('Remove Police user page', () => {
  ...
  describe('Success Page', () => {
    ...
    test('POST /admin/users/police/remove route returns 200 and success page given user has been submitted', async () => {
      const options = {
        method: 'POST',
        url: '/admin/users/police/remove',
        auth,
      ...
      expect(removeUser).toHaveBeenCalledWith(95, user)
      expect(response.statusCode).toBe(200)
      ...
      expect(document.querySelector('#police-user').textContent.trim()).toContain('nicholas.angel@sandford.police.uk')
