...
describe('Statistics route', () => {
  ...
  test('GET /admin/statistics route returns 200', async () => {
    ...
    expect(rows[5].querySelectorAll('.govuk-table__header')[0].textContent.trim()).toBe('Applying for exemption')
    expect(rows[5].querySelectorAll('.govuk-table__cell')[0].textContent.trim()).toBe('30')
    expect(rows[6].querySelectorAll('.govuk-table__header')[0].textContent.trim()).toBe('Failed to exempt dog')
    ...
    expect(rows[9].querySelectorAll('.govuk-table__header')[0].textContent.trim()).toBe('Withdrawn by owner')
